...
//umiesza wartości losowe w kolejce
void InsertRandValues(const string &NameFile, Queue &Object) {
  ifstream InputFile;          //utwórz strumień wejściowy dla pliku
  int random_number;              //wartość odczytna z pliku
  int range = 0 ;               //ilość elementów w pliku
  InputFile.open (NameFile , fstream::in );
  for (; !InputFile.eof(); ++range) {
    InputFile >> random_number;
    Object.enqueue(random_number);    //wpisanie liczby na stos
  }
  InputFile.close();                         //zamknij strumień
}
