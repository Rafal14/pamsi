...
int Heap::insert(const int &odleg, const int &wierzch )
{
  ...
  //przeszukaj całą tablicę kopca i sprawdź czy dodawany wierzchołek
  //ma mniejszą wagę wpisanych wierzchołków
  while ( i > 1 && (  (tab[IndFather(i)].waga) > odleg ) ) {

    x =  tab[i].waga;
    y =  tab[i].wierzcholek;
  
    //zamień wpisywany wierzchołek z jego ojcem
...
int Heap::removeMin()
{
  ...
  return min;                   //zwróć indeks wierzchołka o najmniejszej wadze 
}
...
void Heap::BuildHeap(const int &wym)
{
  ...
  tab = new Node [wym];
  tab[0].waga = 0;              // pierwszy element jest pusty
}


//zwraca ilość elementów znajdujących się w kopcu
int Heap::Size ()
{
  return lenght;
}
